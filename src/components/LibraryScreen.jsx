...
export default function LibraryScreen({ onSelectVideo }) {
  ...
  useEffect(() => {
    fetch('/library.json')
      ...
      .catch(() => {
        setError('Could not load library. Make sure library.json is in the /public folder.')
        setLoading(false)
      })
  }, [])
  ...
  const filtered = activeTags.size === 0
    ? library
    : library.filter(v => [...activeTags].every(t => v.tags?.includes(t)))
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: showTagFilter ? 8 : 0 }}>
          <button
            onClick={() => setShowTagFilter(s => !s)}
            style={{
              display: 'flex', alignItems: 'center', gap: 6,
              background: showTagFilter ? 'var(--accent-ref)' : 'var(--bg-raised)',
          ...
          {activeTags.size > 0 && (
            <button
              onClick={clearTags}
              style={{ fontSize: 12, color: 'var(--accent-global)', background: 'none', border: 'none', padding: '4px 2px' }}
            >
              Clear all
            </button>
          )}
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    background: active ? 'var(--accent-ref)' : 'var(--bg-raised)',
      ...
      <div style={{ 
        ...
        {!loading && activeTags.size > 0 && (
          <>
            ...
            {filtered.map(video => (
              <VideoRow key={video.id} video={video} onSelect={onSelectVideo} activeTags={activeTags} />
            ))}
          </>
        )}

        {!loading && activeTags.size === 0 && categories.map(cat => {
            ...
            <div key={cat} style={{ marginBottom: 6 }}>
              ...
              >
                ...
                <span style={{ color: 'var(--text-muted)', fontSize: 12, display: 'flex', alignItems: 'center', gap: 6 }}>
                  ...
                  <span style={{ transform: isOpen ? 'rotate(180deg)' : 'none', display: 'inline-block', transition: 'transform 0.15s' }}>▾</span>
                </span>
              </button>
              ...
              {isOpen && (
                <div style={{
                  ...
                  {videos.map((video, i) => (
                    <VideoRow key={video.id} video={video} onSelect={onSelectVideo} noBorderTop={i === 0} activeTags={activeTags} />
                  ))}
                </div>
              )}
            </div>
...
function VideoRow({ video, onSelect, noBorderTop, activeTags }) {
  return (
    ...
    >
      <div style={{
        width: 64, height: 42, borderRadius: 'var(--radius-sm)',
        background: 'var(--bg-raised)',
        border: '0.5px solid var(--border)',
        flexShrink: 0,
        display: 'flex',
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: 13, fontWeight: 500, color: 'var(--text-primary)', marginBottom: 3, lineHeight: 1.3 }}>
          {video.title}
        </div>
            ...
            {video.tags.map(tag => (
              <span
                ...
                style={{
                  ...
                  padding: '1px 6px',
                  borderRadius: 10,
                  background: activeTags?.has(tag) ? 'var(--accent-ref-dim)' : 'var(--bg-raised)',
                  color: activeTags?.has(tag) ? 'var(--accent-ref)' : 'var(--text-muted)',
        ...
        {video.duration && (
          <div style={{ fontSize: 11, color: 'var(--accent-ref)', fontFamily: 'var(--font-mono)' }}>
            {video.duration}
          </div>
        )}
      </div>
